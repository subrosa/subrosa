 ...
 */
public final class NotificationListHas extends TypeSafeDiagnosingMatcher<List> {
     ...
     */
    @Factory
    public static Matcher<List> hasNotification(AbstractNotificationMatcher matcher) { // SUPPRESS CHECKSTYLE IllegalType
        return new NotificationListHas(matcher);
    }

    @Override
    protected boolean matchesSafely(List list, Description description) {
        boolean matches = true;
        if (!hasItem(matcher).matches(list)) {
            hasItem(matcher).describeMismatch(list, description);
            matches = false;
        }
        return matches;
    }
     ...
     */
    public abstract static class AbstractNotificationMatcher extends TypeSafeDiagnosingMatcher<Map> { }
